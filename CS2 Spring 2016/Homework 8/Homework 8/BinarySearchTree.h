...
private:
	...
	void deletefromTree(nodeType<elemType>* &p, const elemType deleteItem);
	nodeType<elemType>* findMin(nodeType<elemType> *p);
	void printnode(nodeType<elemType> *p, const elemType& searchItem);
public:
	BinarySearchTree();
	...
	void PrintSingleNode(const elemType &searchItem);
	~BinarySearchTree();
};
...
template <class elemType>
void BinarySearchTree<elemType>::PrintSingleNode(const elemType &searchItem)
{
	printnode(root, searchItem);
}
template <class elemType>
...
{
	deletefromTree(root, deleteItem);
}
template <class elemType>
BinarySearchTree<elemType>::~BinarySearchTree()
{
...
template <class elemType>
nodeType<elemType>*
BinarySearchTree < elemType >::findMin(nodeType<elemType> *p)
{
...
template <class elemType>
void BinarySearchTree<elemType>::deletefromTree(nodeType<elemType> *
	&p, const elemType deleteItem)
{
	if (p == nullptr)
		cout << "Error: The node to be deleted does not exists." << endl;
	else if (p->info == deleteItem)
	{
		...
		{
			nodeType<elemType> *temp = findMin(p->rLink);
			p->info = temp->info;
			deletefromTree(p->rLink, temp->info);
		}
	}
	else if (deleteItem < p->info)
		deletefromTree(p->lLink, deleteItem);
	else
		deletefromTree(p->rLink, deleteItem);
}
template<class elemType>
void BinarySearchTree<elemType>::printnode(nodeType<elemType>* p, const elemType& searchItem)
{
	if (p == nullptr)
		...
		printnode(p->lLink, searchItem);
	else
		 printnode(p->rLink, searchItem);
}